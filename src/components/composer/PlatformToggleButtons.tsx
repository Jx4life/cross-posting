...
export const PlatformToggleButtons: React.FC<PlatformToggleButtonsProps> = ({
  ...
  // Group platforms for better organization
  const primaryPlatforms = [
    { key: 'twitter', label: 'X', icon: X, 
      activeClass: "data-[state=on]:bg-black data-[state=on]:text-white data-[state=on]:border-black" },
    ...
    ), activeClass: "data-[state=on]:bg-[#00501E] data-[state=on]:text-white data-[state=on]:border-[#00501E]" },
    ...
    ), activeClass: "data-[state=on]:bg-[#8B5CF6] data-[state=on]:text-white data-[state=on]:border-[#8B5CF6]" },
  ];
  
  const secondaryPlatforms = [
    { key: 'facebook', label: 'Facebook', icon: Facebook, 
      activeClass: "data-[state=on]:bg-[#1877F2] data-[state=on]:text-white data-[state=on]:border-[#1877F2]" },
    { key: 'instagram', label: 'Instagram', icon: Instagram,
      activeClass: "data-[state=on]:bg-gradient-to-r data-[state=on]:from-[#833AB4] data-[state=on]:via-[#FD1D1D] data-[state=on]:to-[#FCAF45] data-[state=on]:text-white data-[state=on]:border-transparent" },
    { key: 'tiktok', label: 'TikTok', icon: TiktokIcon,
      activeClass: "data-[state=on]:bg-black data-[state=on]:text-white data-[state=on]:border-black" },
    { key: 'youtubeShorts', label: 'YouTube', icon: Youtube,
      activeClass: "data-[state=on]:bg-[#FF0000] data-[state=on]:text-white data-[state=on]:border-[#FF0000]" },
  ];

  // Create a responsive toggle button
  const renderToggleButton = (platformData: any) => {
    return (
      <Toggle 
        key={platformData.key}
        pressed={platforms[platformData.key as keyof PlatformSettings]}
        onPressedChange={(enabled) => handlePlatformToggle(platformData.key as keyof PlatformSettings, enabled)}
        className={cn(
          "border-[1px] border-gray-300 dark:border-gray-700 flex items-center gap-1.5",
          platformData.activeClass,
          isMobile ? "flex-1 min-w-[80px] justify-center text-xs px-1.5 py-1" : ""
        )}
      >
        <platformData.icon />
        {(!isMobile || platformData.key === 'twitter') && <span>{platformData.label}</span>}
      </Toggle>
    );
  };
  ...
  return (
    <div className="flex flex-col space-y-2">
      <div className="flex flex-wrap gap-1.5 md:gap-2">
        {primaryPlatforms.map(renderToggleButton)}
      </div>
      
      <div className="flex flex-wrap gap-1.5 md:gap-2">
        {secondaryPlatforms.map(renderToggleButton)}
      </div>
    </div>
  );
};
